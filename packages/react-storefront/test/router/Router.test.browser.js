...
describe('Router:Browser', function() {
  ...
  describe('runAll', function() {
    ...
    it('should support splat', async function() {
      ...
      expect(await runAll('get', '/products/1/foo')).to.deep.equal({ id: "1", seoText: "foo", ...allRouteData })
      expect(await runAll('get', '/products/1')).to.deep.equal({ id: "1", seoText: undefined, ...allRouteData })
    })
    ...
    it('should support optional params', async function() {
      router.get('/products/:id(/:foo)', handler)
      expect(await runAll('get', '/products/1/2')).to.deep.equal({ id: "1", foo: "2", ...allRouteData })
      expect(await runAll('get', '/products/1')).to.deep.equal({ id: "1", foo: undefined, ...allRouteData })
    })
    ...
    it('should capture the suffix', async function() {
      ...
      expect(await runAll('get', '/users/1.html')).to.deep.equal({ id: '1', format: 'html', ...allRouteData })
    })
    ...
    it('should set loading: false during initialLoad', async () => {
      router.get('/', 
        fromServer((params, request, response) => ({ foo: 'bar' }))
      )
  ...
  describe('use', function() {
    ...
    it('should accept params', async function() {
      ...
      expect(await runAll('get', '/products/1/reviews/2')).to.deep.equal({ id: '1', reviewId: '2', ...allRouteData })
    });
    ...
    it('should accept infinite levels of nesting', async function() {
      ...
      expect(await runAll('get', '/products/1/reviews/2')).to.deep.equal({ productId: '1', reviewId: '2', ...allRouteData })
    })

    it('should match based on extension', async function() {
      router.use('/c', new Router()
        .get('/:id.html', fromClient(() => Promise.resolve('html')))
      );
